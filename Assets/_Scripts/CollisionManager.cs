...
public class CollisionManager : MonoBehaviour
{
    ...
    void Update()
    {
        bullets = FindObjectsOfType<CustomBullet>();
    ...
    private static void ReflectCustom(CustomBullet s)
    {
        if ((s.collisionNormal == Vector3.forward) || (s.collisionNormal == Vector3.back))
        {
            s.direction = new Vector3(s.direction.x, s.direction.y, -s.direction.z);
    ...
    public static void CheckAABBs(CubeBehaviour a, CubeBehaviour b)
    {
            ...
            (a.min.z <= b.max.z && a.max.z >= b.min.z))
        {
            ...
            if (!a.contacts.Contains(contactB))
            {
                ...
                //collision:
                if (b.gameObject.GetComponent<RigidBody3D>().bodyType == BodyType.STATIC && a.gameObject.name != "Player")
                {
                    a.gameObject.GetComponent<RigidBody3D>().Stop();
                }
                else
                {
                    ...
                    {
                        a.gameObject.GetComponent<RigidBody3D>().Stop();
                        a.gameObject.GetComponent<RigidBody3D>().PushX(
                            b.gameObject.GetComponent<RigidBody3D>().velocity.x * b.gameObject.GetComponent<RigidBody3D>().mass);
                        a.gameObject.GetComponent<RigidBody3D>().PushZ(
                            b.gameObject.GetComponent<RigidBody3D>().velocity.z * b.gameObject.GetComponent<RigidBody3D>().mass);
    ...
    public static void CheckCustomAABBs(CustomBullet a, CubeBehaviour b)
    {
        Contact contactB = new Contact(b);
